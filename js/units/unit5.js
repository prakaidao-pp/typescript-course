...
// 😱 แล้วถ้ามี 100 คน ต้องเขียน 100 บรรทัด!</code></pre>
    ...
    right: `
      ...
      <div class="info-box info-box--info" style="margin-top: var(--space-sm);">
        <div class="info-box__title">💡 ลองเปรียบกับชั้นวางรองเท้า (Shoe Rack)</div>
        <div class="info-box__content" style="font-size: 13px; line-height: 1.5;">
          ถ้ามีรองเท้า 5 คู่ วางกล่องรองเท้าแยกกันกระจายทั่วบ้าน (เหมือนตัวแปรแยกกัน) จะจัดการยากและลืมง่าย แต่ถ้ามี <strong>ชั้นวางรองเท้า 1 หลังที่มี 5 ช่อง (เหมือน Array)</strong> เราจะเก็บทุกคู่รวมกันในที่เดียวอย่างระเบียบและหยิบง่ายครับ
        </div>
      </div>
  ...
  },
  {
    ...
    content: `
      ...
      <div class="info-box info-box--warning" style="margin-top: var(--space-md);">
        <div class="info-box__title">❓ ทำไม Index ถึงเริ่มจาก 0?</div>
        <div class="info-box__content">
          ให้นึกถึง <strong>"ตึกที่มีชั้น Ground (ชั้น G)"</strong> เป็นชั้นเริ่มต้น (ชั้น 0) หรือ <strong>"ไม้บรรทัด"</strong> ที่เริ่มต้นวัดระยะจากขีดเลข 0 เสมอ!<br>
          ในทางคอมพิวเตอร์ Index คือ <strong>"ระยะห่างจากจุดเริ่มต้น" (Offset)</strong>:<br>
          - ช่องแรก (index 0) = ห่างจากจุดเริ่มต้น 0 ก้าว<br>
          - ช่องที่สอง (index 1) = ห่างจากจุดเริ่มต้น 1 ก้าว
        </div>
      </div>
    `
  },
...
let product:    [string, number, boolean] = ["กาแฟ", 65, true];  // ชื่อ, ราคา, มีสินค้า`,
    lang: 'typescript',
    note: '<strong>🎁 เปรียบเทียบ Tuple: กล่องชุดของขวัญสำเร็จรูป (Gift Box Set)</strong><br>ถ้า Array ธรรมดาคือ "ตู้ลิ้นชักที่ทุกช่องต้องเก็บเสื้อยืดเหมือนกันหมด" <strong>Tuple</strong> จะเหมือนกับ <strong>"กล่องชุดของขวัญ (Gift Box Set)"</strong> ที่มีบล็อกล็อกตำแหน่งเฉพาะตัวที่ห้ามเปลี่ยน เช่น ช่องแรกใส่แก้วน้ำ (string), ช่องสองใส่ช้อนส้อม (number) และช่องสามใส่การ์ดอวยพร (boolean) คุณไม่สามารถเอาของผิดประเภทไปยัดใส่ช่องของมันได้ และลำดับก็ต้องเป๊ะตามที่จัดไว้ตั้งแต่แรกครับ'
  },
  {
    type: 'code',
...
console.log(stack);    // ["A", "B"]`,
    lang: 'typescript',
    note: '<strong>🎾 ลองเปรียบเทียบกับ: หลอดใส่ลูกเทนนิส หรือ กระปุกมันฝรั่งทอด (Pringles)</strong><br>- <strong><code>push()</code> (ดันเข้า)</strong>: เหมือนเวลาเราหย่อนลูกเทนนิสลูกใหม่ลงไปในหลอดเก็บ มันจะตกลงไปต่ออยู่ด้านบนสุด (ท้ายแถว) เสมอ<br>- <strong><code>pop()</code> (เด้งออก)</strong>: เหมือนเวลาเราหยิบลูกเทนนิสออกจากปากหลอด เราจะหยิบได้เฉพาะลูกที่อยู่ด้านบนสุด (ตัวล่าสุดที่เพิ่งใส่เข้าไป) ออกมาได้ก่อนเท่านั้น'
  },
  {
    type: 'code',
...
console.log(greetings); // ["สวัสดี, สมชาย!", "สวัสดี, สมหญิง!", "สวัสดี, สมศักดิ์!"]`,
    lang: 'typescript',
    note: '<strong>🏭 เปรียบเทียบ map(): สายพานโรงงานแปรรูปผลไม้ (Conveyor Belt)</strong><br>ลองนึกภาพสายพานที่มีผลไม้ดิบเรียงกันมา 3 ชิ้น: <code>[กล้วยดิบ, มะม่วงดิบ, มะละกอดิบ]</code> เมื่อวิ่งผ่านเครื่องแปรรูปจะได้ <code>[กล้วยสุก, มะม่วงสุก, มะละกอสุก]</code> <strong>จำนวนผลไม้ยังมี 3 ชิ้นเท่าเดิม</strong> ไม่เพิ่มไม่ลด แต่สถานะถูกเปลี่ยนไปทั้งหมด นี่คือ <code>map()</code> ครับ'
  },
  {
    type: 'code',
...
console.log(gFruits);   // ["กล้วย", "กีวี", "แคนตาลูป"]`,
    lang: 'typescript',
    note: '<strong>🚪 เปรียบเทียบ filter(): ตะแกรงร่อน หรือ ประตูกรองความสูงสวนสนุก</strong><br>ลองนึกภาพเด็กๆ 5 คนมาต่อคิวเล่นเครื่องเล่น แต่ตรงทางเข้ามี "ประตูกรองความสูง" (เช่น ส่วนสูง >= 120 ซม.) เด็กที่ผ่านเกณฑ์เท่านั้นที่จะได้เข้าไปเล่นต่อ ผลลัพธ์สุดท้ายคือนักเรียนกลุ่มใหม่ที่ <strong>อาจจะมีจำนวนน้อยลง หรือไม่มีเลย</strong> แต่ทุกคนในกลุ่มใหม่นั้นผ่านเงื่อนไขทั้งหมดเหมือนกัน'
  },
  {
    type: 'content',
  ...
  },
  {
    ...
    content: `
      ...
      <div class="info-box info-box--info" style="margin-top: var(--space-md);">
        <div class="info-box__title">🍊 เปรียบเทียบ Function: เครื่องคั้นน้ำส้มอัตโนมัติ (Juicer Machine)</div>
        <div class="info-box__content">
          ลองจินตนาการถึงเครื่องคั้นน้ำส้มกล่องหนึ่งที่เราสร้างขึ้นมา ภายในเครื่องมีขั้นตอนเพียบเลย (ล้างส้ม -> ผ่าครึ่ง -> คั้นน้ำ -> แยกเมล็ด -> เทใส่แก้ว) หากเราไม่มีเครื่องนี้ ทุกครั้งที่อยากกินน้ำส้ม เราต้องทำทีละขั้นตอนด้วยตัวเองทุกครั้ง (เขียนโค้ดซ้ำๆ)<br>
          แต่เมื่อเรามี <strong>เครื่องคั้นน้ำส้ม (Function)</strong> แล้ว ต่อไปนี้เราแค่โยน <strong>ส้ม (Parameter/วัตถุดิบ)</strong> เข้าไปในเครื่อง แล้วกดปุ่ม เครื่องก็จะทำงานทั้งหมดแทนเรา และส่ง <strong>น้ำส้ม (Return value/ผลลัพธ์)</strong> ออกมาให้เราดื่มได้ทันที!
        </div>
      </div>
      <div class="info-box info-box--info" style="margin-top: var(--space-md);">
...
greetDefault();          // "สวัสดี, แขกผู้มาเยือน!"`,
    lang: 'typescript',
    note: '<strong>🥕 เปรียบเทียบ Parameter: วัตถุดิบที่คุณใส่เครื่องปั่น</strong><br>หากคุณสร้างเครื่องทำสมูทตี้ (Function) แล้วโยน <strong>สตรอว์เบอร์รี่</strong> เข้าไป (Argument) -> จะได้รสสตรอว์เบอร์รี่ โยน <strong>มะม่วง</strong> เข้าไป -> จะได้รสมะม่วง ดังนั้น Parameter คือ "ช่องใส่ของที่เตรียมไว้" ส่วนข้อมูลจริงที่โยนลงไปตอนรันเรียกว่า "Argument" ครับ'
  },
  {
    type: 'code',
    title: 'Return Type',
...
console.log(fullName("สมชาย", "ใจดี")); // "สมชาย ใจดี"`,
    lang: 'typescript',
    note: '<strong>🎁 ทำความเข้าใจเรื่อง Return vs Void:</strong><br>- <strong>มี <code>return</code> (ส่งของกลับมา)</strong>: เหมือนเครื่องปิ้งขนมปังที่คุณใส่ขนมปังดิบเข้าไป แล้วคุณรอรับ <strong>ขนมปังกรอบ (ผลลัพธ์)</strong> เด้งออกมาเพื่อนำไปกินต่อได้<br>- <strong>ไม่มี return หรือ <code>void</code> (ไม่ส่งของกลับ)</strong>: เหมือนตู้เย็นที่คุณแช่ของเฉยๆ หรือทีวีที่แสดงผลทางจอภาพเฉยๆ ไม่ได้ส่งค่ากลับคืนออกมาให้ทำงานต่อในโค้ดส่วนอื่น'
  },
  {
    type: 'code',
